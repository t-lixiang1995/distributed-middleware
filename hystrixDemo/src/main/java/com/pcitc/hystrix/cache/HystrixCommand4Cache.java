package com.pcitc.hystrix.cache;
...
/**
 * Hystrix 请求缓存(Hystrix可将请求进行缓存，当后续有相同的请求时，直接返回缓存中的响应。从而避免直接对服务进行调用，增加服务的压力。)
 * 1.如何判断请求是否从缓存获取值? ---根据请求参数判断,如果参数相同,则返回值相同,可取缓存结果
 * 2.HystrixRequestContext.initializeContext()开启请求上下文，只有在同一个上下文中的请求才会获得缓存
 *
 * @author : pcitc
